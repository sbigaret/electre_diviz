...
"""
ElectreTriCredibility - computes credibility matrix as presented in Electre TRI
method.

It uses the same procedure as in ElectreTriCCredibility - only the input files
...
ElectreTriDiscordances) is equal to ElectreTriExploitation module split into
four separate parts for user's convenience.

Usage:
    ElectreTriCredibility.py -i DIR -o DIR
...
def get_credibility(concordance, discordances, alternatives, categories_profiles):
    ...
    concordance_ap, concordance_pa = unreverseAltComparisons(
        concordance,
        alternatives,
        categories_profiles,
    )
    discordances_ap, discordances_pa = unreverseAltComparisons(
        discordances,
        alternatives,
        categories_profiles,
    )
